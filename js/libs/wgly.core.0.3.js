;var gz = (function () {
    ...
    var 
        ...
        bf = new ArrayBuffer(4),
        i8  = new Uint8Array(bf),
        f32 = new Float32Array(bf),
        i32 = new Uint32Array(bf);
    ...
    function _co(str, on_finish, on_progress, mode) {
        var this$static = {},
            percent,
            cbn,
        ...
        if (sync) {
            this$static.c = $LZMAByteArrayCompressor({}, c, get_mode_obj(mode));
    ...
    function _do(byte_arr, on_finish, on_progress) {
        var this$static = {},
            percent,
            cbn, 
            has_progress,
...
Fl.prototype = {
    ...
    getByts:function(l){
        this.i += ( l = l || 1 );
        return this.d.slice( this.i - l, this.i );
    },
    getInt32:function(){
...
var ut = {
    ...
    f32toi8:function(a){ return new Uint8Array( new Float32Array(a).buffer ) },
    i8tof32:function(a){ return new Float32Array( new Uint8Array(a).buffer ) },
    joinF:function(a){
        var i=0,l1=a.length/4,s,e,m,_bf = new Float32Array( l1 ),
        off = l1*3;
...
//          i8[0] = a[i*4];
//          i8[1] = a[i*4+1];
//          i8[2] = a[i*4+2]&0x7f | (a[i*4+3]<<7);
//          i8[3] = (a[i*4+3]>>1) | (a[i*4+2]&0x80);
            
            i8[0] = a[i*3];
            i8[1] = a[i*3+1];
            i8[2] = a[i*3+2]&0x7f | (a[i+off]<<7);
            i8[3] = (a[i+off]>>1) | (a[i*3+2]&0x80);
    ...
    splitF:function(a){
        var i=0,l1,l,h,_bf = new Uint8Array(a.length*4),off=a.length*3;
        for(l1=a.length;i<l1;i++){
            
            f32[0] = a[i];
            
            l = (i8[3]>>7<<7)|(i8[2]&0x7f);
            h = ( i8[3]<<1 ) | ( i8[2]>>7 );
...
//          _bf[i * 4 + 3 ] = h;
    ...
    enSort:function( arr, size ){
      if( isNaN(size) || size<=1 ) return arr;
      var as = function(l){
        var _ = [];
        while(l-->0)_.push([]);
        return _;
      ...
      , idx=0
      , i = 0
      , l = Math.ceil( arr.length/size) ;

      for(;i<l; i++ ,tmp = 0 ){
        while(tmp <size ){
          if( al > (idx = i*size + tmp ) )
          as[tmp].push( arr[ idx ] );
          tmp++
        }
      }
      return new Float32Array( Array.prototype.concat.apply(as[0], as.slice(1)) )
    },
    deSort:function( arr, size ){
      if( isNaN(size) || size<=1 ) return arr;
      var al = Math.ceil( arr.length/size)
      , l = arr.length
      , tmp =0
      , as = new Array(l)
      , ii = 0
      ...
      for(;i<al; i++ ,tmp = 0){
        while(tmp<size){
          if(ii<l) as[ ii++ ] =  arr[ tmp*al + i ] ;
          tmp++;
        }
      }
      return new Float32Array(as);
    },
    setLength:function( l ){
        var i2 = (l).toString(2);
        var ls = Math.ceil( i2.length/7 );
        var _bf = new Uint8Array(ls);
        for(var i=0;i<ls;i++){
            _bf[i]= l>>(i*7)&0x7f;
            if(i==(ls-1)) _bf[i]=_bf[i]|0x80;
        }
        return _bf
    },
    getLength:function( data,i ){
        var t,l=0,z,s,ii=0;
...
//  console.log(l,i,ii)
    ...
    enHead:function( ty, n, data ){
        
        var bl = ut.setLength(data.byteLength), bf ;

        if(typeof n=='string'&&n.length>0){
            var nf = ut.enName(n);
            bf = new            Uint8Array(nf.length+bl.length+data.byteLength+2);
            bf[0]=ty<<1|1;
            bf[1]=n.length;
            bf.set(nf,2);
            bf.set(bl,nf.length+2);
            bf.set(data,bl.length+nf.length+2);
        }else{
            bf = new Uint8Array(bl.length+data.byteLength+1);
            bf[0]=ty<<1;
            bf.set(bl,1);
            bf.set(data,bl.length+1);
        }
        
        return bf
    },
    deHead:function( data, i ){
        
        i = i || 0;
        if(i>=data.length) return false;
        
        var ty = data[i]>>1,idx,
            nl = data[i]&1,n=null,l,fl;
        
        if(nl){
            idx = i+2;
            n = ut.deName(data.subarray(idx,idx=idx+data[i+1]));
        }else{
            idx = i+1;
        }
                
        l = ut.getLength(data,idx);
        
        fl = new Fl( data.subarray( l.i,idx = l.l+l.i) );
        fl.n = n;
        fl.t = ty;
...
//       console.log(idx,l)
    ...
    enName:function(n){
        for(var i=0,l=n.length,bf=new Uint8Array(l);i<l;i++)
            bf[i]=n.charCodeAt(i)
        return bf
    },
    deName:function( d ){
    ...
    enInd:function( a, n ){
        ...
        var max=a[0],as = [max],t,ty,bf;
        for(var i=1,l=a.length;i<l;i++){
            
            as.push( Math.abs( t = a[i]-a[i-1] )>max?max=t:t );
        ...
        if(max<=128){
            ...
            bf = new Uint8Array( (new Int16Array(as)).buffer );
            ty = TYPE.I16;
        }else{
            bf = new Uint8Array( (new Int32Array(as)).buffer ); 
            ty = TYPE.I32;
        }
        
//      console.log(as)
        
        return ut.enHead( ty, n, bf );
    },
    deInd:function( fl ){
        
        var t =fl.t,
            a =new (t==TYPE.I8&&Int8Array||
                    t==TYPE.I16&&Int16Array||
                    Int32Array)(fl.d);
        
//      console.log(a)
        
        var as = new Uint16Array(a.length);
        as[0]=a[0];
        for(var i=1,l=a.length;i<l;i++)
        as[i]= a[i] + as[i-1];
        fl.cd = as;
    ...
    enGeo:function( o, n ){
        ...
        var tmp,arr=[];
        if( tmp = o.vertices || o.positions )       arr.push(ut.enVec3(tmp,1,TYPE.VERT));
        if( tmp = o.indices ) arr.push(ut.enInd(tmp));
        if( tmp = o.normals ) arr.push(ut.enVec3(tmp,1,TYPE.NORM));
        if( tmp = o.uvMaps || o.uvs || o.uv )       arr.push(ut.enVec2(tmp,1,TYPE.UV));
        
//      console.log(arr)
  
        return ut.enHead( TYPE.GEO, n, ut.joinFl(arr) )

    },
    deGeo:function( fl ){
//      console.log(fl)
        var ar = ut.splitFl(fl.d),mesh={};
        ar.map(function(m){
            switch(m.t){
                case TYPE.I8:
                case TYPE.I16:
                case TYPE.I32:ut.deInd(m);
                    mesh.indices = m.cd;
                    break;
                case TYPE.VERT:ut.deVec3(m);
                    mesh.vertices = m.cd;
                    break;
                case TYPE.NORM:ut.deVec3(m);
                    mesh.normals = m.cd;
                    break;
                case TYPE.UV:ut.deVec2(m);
                    mesh.textures = m.cd;
                    break;
                default:break;
            }
    ...
    enString:function( str, n ){

        // var codes = [];
        // for (var i = 0; i < str.length; i++) {
        //     codes[i + 1] = parseInt(str[i].charCodeAt(0), 10);
        // }
        // return ut.enHead( TYPE.ST, n, new Uint8Array( (new      Uint16Array(codes)).buffer ) );
        return ut.enHead( TYPE.ST, n, new Uint8Array( encode( str ) ) );
    },
    deString:function( fl ){
        // var s = '';
        // var it = new Uint16Array( fl.d );
        // for(var i = 0,l=it.length;i<l;i++){
        //     s+= String.fromCharCode(it[i])
        // }
    ...
    enFloat:function( bf, n ,ty){
        return ut.enHead(ty||TYPE.VEC3,n,ut.splitF( bf ) )
    },
    deFloat:function( fl ){
        fl.cd = ut.joinF(fl.d);return fl
    },
    enVec2:function( bf, n , ty ){
        return ut.enHead(ty||TYPE.VEC2,n,ut.splitF(ut.enSort(bf,2)))
    },
    deVec2:function( fl ){
        fl.cd = ut.deSort( ut.joinF(fl.d),2 );return fl
    },
    enVec3:function( bf, n , ty ){
        return ut.enHead(ty||TYPE.VEC3,n,ut.splitF(ut.enSort(bf,3)))
    },
    deVec3:function( fl ){
        fl.cd = ut.deSort( ut.joinF(fl.d) ,3 );return fl
    },
    joinFl:function(a){
        
        var l =0;
        a.map(function(m){l+=m.length});
        var bf = new Uint8Array(l);
        l=0;
        a.map(function(m){
            bf.set(m,l);
            l+=m.length;
    ...
    splitFl:function(data){
        var ar=[],t=null,i=0;
        while( t=ut.deHead(data,i) ){
            ar.push(t.f);
            i=t.i;
        }
        return ar
...
function Dwgl( source ){

    var arr = ut.splitFl(source);
    ...
    arr.map(function(m){
        switch(m.t){
            ...
            case TYPE.I8:
            case TYPE.I16:
            case TYPE.I32:ut.deInd(m);break;
            case TYPE.VERT:
            case TYPE.VEC3:
            case TYPE.NORM:ut.deVec3(m);break;
            case TYPE.VEC2:
            case TYPE.UV:ut.deVec2(m);break;
            case TYPE.FL:ut.deFloat(m);break;
            case TYPE.ST:ut.deString(m);break;
            case TYPE.GEO:ut.deGeo(m);break;
            default:break;
        }
    });
    this.datas = arr;
...
Dwgl.prototype = {
    constructor:Dwgl,
    getById:function(i){
        return i<this.data.length?this.datas[i]:undefined;
    },
    getByName:function(n){
        var arr=[];
        this.datas.map(function(m){
            if(m.n == n)arr.push(m)
        });
        return arr
    },
...
function Cwgl(){
    ...
    this.data = null;
    this.pend = [];
    this.state = STATE.NOCOM;
}

Cwgl.prototype = {
    constructor:Cwgl,
    add:function(c){
        if(this.state>STATE.NOCOM)
            return console.log('已经不能添加数据!');
        if(!c.data||!c.type){
            console.log('添加的数据格式不对!');
            return
        }
        this.pend.push(c);
        return this.pend.length-1; 
    },
    adds:function(arr){
        if(this.state>STATE.NOCOM)
            return console.log('已经不能添加数据!');
        var s = this.pend.length;
        for(var i=0,l=arr.length;i<l;i++)
        this.add(arr[i]);
        return this;
    },
    compile:function(c,p){
        var a=[];
        this.pend.map(function(m){
            switch(m.type){
                case TYPE.FL:
                a.push(ut.enFloat(m.data,m.name,m.type));break;
                case TYPE.VEC2:
                case TYPE.UV:
                a.push(ut.enVec2(m.data,m.name,m.type));break;
                case TYPE.VEC3:
                case TYPE.VERT:
                case TYPE.NORM:
                a.push(ut.enVec3(m.data,m.name,m.type));break;
                case TYPE.ST:
                a.push(ut.enString(m.data,m.name,m.type));break;
                case TYPE.GEO:
                a.push(ut.enGeo(m.data,m.name,m.type));break;
                default:break;
            }
        });
        _co(ut.joinFl(a),c,p)
...
function parseGro(g){
    ...
    if(g.data.attributes.position)
        re.data.vertices = g.data.attributes.position.array;

    if(g.data.attributes.normal)
        re.data.normals = g.data.attributes.normal.array;

    if(g.data.attributes.uv)
        re.data.uvs = g.data.attributes.uv.array;

    if(g.data.index)
        re.data.indices = g.data.index.array;
...
function decGeo(g,f){
    ...
    var ge = f.getByName(g.uuid);
    if(ge.length>0)ge = ge[0].cd;
    else return;
    ...
    if(ge.normals)g.data.attributes.normal = {
        array:ge.normals,
        itemSize:3,
        type:"Float32Array"
    };

    if(ge.vertices)g.data.attributes.position = {
        array:ge.vertices,
        itemSize:3,
        type:"Float32Array"
    };

    if(ge.textures)g.data.attributes.uv = {
        array:ge.textures,
        itemSize:2,
        type:"Float32Array"
    };

    if(ge.indices)g.data.index = {
        array:ge.indices,
        type:"Uint16Array"
    };
...
function enjson(js,c,p){

    var ar = [];

    if(js.geometries){

        js.geometries.map(function(m){
...
function dejson(a,c,p){

    _do(a,function(d){

        var dd = new Dwgl( new Uint8Array(d));
        var s = dd.getByName('gljson')[0].cd;
       
        var json =  JSON.parse( s );


        json.geometries.map(function(m){
            decGeo(m,dd)
        });
...
return {
    enGz:function(a,c,p){ enjson(a,c,p) },
    unGz:function(a,c,p){ dejson(a,c,p) }
}          
